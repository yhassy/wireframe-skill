...
- **Direction toggle** — click the ↕/↔ button on any container's grab bar to switch between vertical and horizontal layout
- **Copy & paste** — click to select an element, then ⌘C/⌘V (Ctrl+C/V) to duplicate it after the selected element. Nested containers are deep-copied.
- **Delete** — select an element and press Delete/Backspace to remove it
- **Undo / Redo** — ⌘Z (Ctrl+Z) to undo, ⌘⇧Z (Ctrl+⇧Z) to redo
...
- **レイアウト方向の切り替え** — コンテナのgrab barにある↕/↔ボタンをクリックして、vertical⇔horizontalを切り替え
- **コピー&ペースト** — 要素をクリックして選択し、⌘C/⌘Vで選択要素の直後に複製。ネストされたコンテナも子要素ごとディープコピー。
- **削除** — 要素を選択してDelete/Backspaceキーで削除
- **元に戻す／やり直し** — ⌘Z（Ctrl+Z）で取り消し、⌘⇧Z（Ctrl+⇧Z）でやり直し
